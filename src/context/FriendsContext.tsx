...
export const FriendsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loadFriendsData = async () => {
    ...
    setIsLoading(true);
    try {
      // Load friends and friend requests first in sequence
      await Promise.all([
        loadFriends(),
        loadFriendRequests()
      ]);
      
      // Small delay to ensure friends state is updated
    ...
    try {
      ...
      const sessionsWithDetails = sessionsData.map(s => {
        ...
        if (!userName && s.user_id === user.id) {
          ...
          // Use current friends state to get name
          const friend = friends.find(f => f.friend_id === s.user_id);
          userName = friend?.friend_name || sessionUser?.name || 'Unknown User';
        }
    ...
    try {
      ...
      const messagesWithDetails = messagesData.map(m => {
        ...
        if (!userName && m.user_id === user.id) {
          ...
          userName = friend?.friend_name || messageUser?.name || 'Unknown User';
        }
